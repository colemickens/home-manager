  ...
  waybarBarConfig = submodule {
    options = {
      layer = mkOption {
        ...
        default = null;
        description = ''
          Decide if the bar is displayed in front (<code>"top"</code>) of the windows or behind (<code>"bottom"</code>).'';
        example = "top";
      };
  ...
  options.programs.waybar = {
    ...
    systemd = {
      ...
      withSwayIntegration = mkOption {
        ...
        example = true;
        description =
          "Whether to bind the systemd service to the sway-session.target instead of graphical-session.target.";
      };
    };
  ...
  in mkIf cfg.enable (mkMerge [
    ...
    (mkIf cfg.systemd.enable {
      systemd.user.services.waybar = {
        ...
        Install = {
          WantedBy = [
            (if cfg.systemd.withSwayIntegration then
              "sway-session.target"
            else
